...
> **项目定位**：为轻量化内容创作者（特别是播客精华提炼者）打造的一站式公众号排版工具，解决“写完内容后还要花大量时间排版配图”的痛点。
...
- **具体任务**：
    ...
    1. 使用 Vite 创建 Vue 3 项目：`npm create vite@latest podcast-publish -- --template vue`
        
    2. 安装并集成 `md-editor-v3`：`npm install md-editor-v3`
    ...
    4. 实现拖拽上传 `.md` 文件功能（使用 HTML5 Drag & Drop API 或 md-editor 自带功能）
...
- **具体任务**：
    ...
    2. 编写 `markdownToWechatHtml(markdownText, themeConfig)` 函数
        
    3. 通过 `markdown-it` 的 `render` 方法获取原始 HTML
    ...
    5. 实现“一键复制 HTML 到剪贴板”功能：
        
        - 使用 `navigator.clipboard.write()` 方法写入带 `text/html` 格式的数据
            
    6. 在右侧预览区用 `v-html` 渲染转换后的 HTML（注意 XSS 防护，可配合 `DOMPurify`）
...
- **具体任务**：
    ...
    2. 预设 3~5 套风格（播客风、极简风、文艺风、科技风）
        
    3. 在界面顶部添加“风格选择下拉菜单”
        
    4. 在右侧预览区或侧边栏添加“高级样式调节面板”：
...
- **具体任务**：
    ...
    1. 注册 Unsplash 开发者账号，创建应用获取 **Access Key**
        
    2. 在界面中加入“配图推荐”按钮或侧边栏
        
    3. 编写前端逻辑：点击后提取文章标题或第一段文本作为关键词
        
    4. 调用 Unsplash Search API：`https://api.unsplash.com/search/photos?query={关键词}&per_page=9`
        
    5. 在界面底部或侧边弹出网格展示返回的图片缩略图
...
- **具体任务**：
    ...
    1. 构建前端静态文件：`npm run build`（生成 `dist` 目录）
        
    2. 新建 Flask 项目（`app.py`），将 `dist` 目录作为静态文件夹托管
        
    3. Flask 提供主要路由 `@app.route('/')` 返回 `index.html`
        
    4. （可选）Flask 提供代理 API，解决 Unsplash 请求跨域问题
        
    5. 编写启动脚本 `run.py` 或 `start.sh`，一键启动 Flask 服务
        
    6. 编写清晰的 README.md 文档，包含：
...
完成该 Demo 后，你可以在简历中这样描述：

> **项目：PodcastPublish —— 面向内容创作者的智能排版工具**
> 
> - **技术栈**：Vue 3、Vite、Python Flask、markdown-it、Unsplash API
>     
> - **核心贡献**：
>     
>     1. 设计并实现了**动态内联样式注入引擎**，解决公众号富文本排版兼容问题，支持 H1~H6 全层级及独立字号调节；
>         
>     2. 构建了**可视化主题系统**，允许用户一键切换/自定义风格，并通过 localStorage 实现持久化；
>         
>     3. 集成 **Unsplash 开放 API**，实现基于文章语义的智能配图推荐，提升创作者效率 50%+；
>         
>     4. 完成从脚本到 **完整 Web 产品 MVP** 的工程化封装，具备完整的交互界面与部署文档。
>         

面试时可以展示项目的 GitHub 仓库、运行截图，甚至现场跑一下 Demo，非常有说服力。

---

## 九、下一步行动（今天就可以开始）

1. **准备环境**：安装 Node.js（LTS 版本）和 Python（3.8+）
    
2. **初始化前端项目**：按照第一阶段任务创建 Vue 3 项目并安装 md-editor
    
3. **画出草图**：不用代码，用笔在纸上画出你想要的界面布局，明确交互逻辑
    
4. **写第一行代码**：搭建 App.vue 的基础框架，让左右栏显示出来
